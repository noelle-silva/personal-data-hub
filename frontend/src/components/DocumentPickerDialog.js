...
const DocumentPickerDialog = ({
  ...
  const [selectedDocumentIds, setSelectedDocumentIds] = useState(initialSelectedIds);
  ...
  const fetchDocuments = useCallback(async (isSearch = false, resetPage = false) => {
    ...
    try {
      ...
      if (err.name !== 'AbortError' && err.name !== 'CanceledError' && err.code !== 'ERR_CANCELED') {
        setError(err.response?.data?.message || err.message || '获取文档失败');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirm = useCallback(() => {
    ...
    handleClose();
  }, [selectedDocumentIds, onConfirm, handleClose]);
  ...
  const renderDocumentItem = (document) => {
    ...
    const isExcluded = excludeIds.includes(document._id);
          ...
          secondary={
            <Box sx={{ mt: 1 }}>
              ...
              <Typography variant="caption" color="text.secondary">
                更新于 {formatRelativeTime(document.updatedAt)}
              </Typography>
            </Box>
          }
        />
  ...
  return (
    <StyledDialog
      ...
      maxWidth="md"
      fullWidth
      aria-labelledby="document-picker-dialog-title"
    >
      <DialogTitle id="document-picker-dialog-title">
      ...
      <DialogActions>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', px: 2, pb: 1 }}>
          <Typography variant="body2" color="text.secondary">
            已选择 {selectedDocumentIds.length} 个笔记
            {total > 0 && ` / 共 ${total} 个`}
          </Typography>
          <Box>
